...
class EmissionAppSetup(TestCase):
    def setUp(self):
        ...
        # Create and log in a user so @login_required views return 200 instead of 302.
        User = get_user_model()
...
class HistoryViewTest(EmissionAppSetup):
    ...
    def test_history_filter_by_activity(self):
        response = self.client.get(reverse("history"), {"activity": str(self.car.id)})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context["records"]), 1)
        self.assertEqual(
            response.context["records"][0].activity.activity_name,
            "Car Travel",
        )
    ...
    def test_delete_record(self):
        initial_count = EmissionRecord.objects.count()
        response = self.client.post(reverse("delete_record", args=[self.record1.id]))
